...
class Database():
    ...
    # dateInfo新增一条数据
    def insertToDateInfo(self, table_name, date, newAdd, newAsymptomatic):
        sql = f"insert into {table_name} values {date, newAdd, newAsymptomatic}"
        try:
            self.__cursor.execute(sql)
            self.__conn.commit()
            print("插入成功")
    ...
    # areaInfo新增数据
    def insertToAreaInfo(self, table_name, area,newAdd,newAsymptomatic,date):
        sql = f"insert into {table_name} values {area,newAdd,newAsymptomatic,date}"
        try:
            self.__cursor.execute(sql)
            self.__conn.commit()
            print("插入成功")
